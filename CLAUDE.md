...
- **`@litmus/core`** — Domain primitives and use case types: `AggregateRoot`, `Entity`, `ValueObject`, `DomainEvent`, `DomainError`, `Repository`, `CommandHandler`, `QueryHandler`. Sub-paths: `./ai` (AiTask, Agent, Toolbox — tools can declare `trustedParams`, hidden from the LLM and bound by the application via `withTrustedValues`), `./db` (DbContext interface), `./id` (prefixedUlid), `./events` (DomainEventDispatcher).
...
- **`@litmus/ai`** — AI SDK adapters. Sub-path `@litmus/ai/vercel` exposes `toVercelTools` for converting picked tools (a `ToolSelection`) to Vercel AI SDK tool format. `ai` is a peer dep.
