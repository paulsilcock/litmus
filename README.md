...
- **`@litmus/test`** — Test utilities for non-deterministic and acceptance testing. `trial()` probabilistic runner, `Grader` type, `UserSimulator`, `useInMemoryTracing()`, `BaseBrowserDriver` for browser ATDD, and the abstract `BaseDriver` lifecycle that entrypoint-specific drivers extend. `playwright` is a peer dep — install with `vp dlx playwright install --with-deps chromium` if using `BaseBrowserDriver`.
