...
- **`@litmus/http`** — HTTP entrypoint adapter wrapping Hono. Exports `routeHandler` (adapts use cases to routes with validation, status defaults, SSE streaming) and `serve` (lifecycle wrapper with init/stop hooks and structured error mapping). Sub-path `@litmus/http/testing` exposes `BaseHttpDriver` and `BaseHonoDriver` for ATDD. `hono` and `@hono/node-server` are peer deps.
- **`@litmus/cli`** — CLI entrypoint adapter. Typed command registration, grouped commands, unix socket transport with typed `cliClient` for RPC-style calls. Sub-path `@litmus/cli/testing` exposes `BaseLitmusCliDriver` for ATDD. `zod` is a peer dep.
...
- **`@litmus/test`** — Test utilities for non-deterministic and acceptance testing. `evaluate()` probabilistic runner, `synthesize()` for fanning out hand-written seeds into more scenarios via an LLM (with hash-keyed file cache; generation opt-in via `LITMUS_SYNTH_MODE=regenerate`), `Grader` type, `UserSimulator`, `useInMemoryTracing()`, `BaseBrowserDriver` for browser ATDD, and the abstract `BaseDriver` lifecycle that entrypoint-specific drivers extend. `playwright` is a peer dep — install with `vp dlx playwright install --with-deps chromium` if using `BaseBrowserDriver`.
